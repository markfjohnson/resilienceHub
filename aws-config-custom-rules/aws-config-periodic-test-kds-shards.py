...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    kds_client = get_client('kinesis', event)
    query = kds_client.list_streams()
    evaluationList = []

    ###HERE I"M USING BOTO3 to CREATE A LIST OF INSTANCE ID'S
    for s in query['StreamNames']:
        kds = kds_client.describe_stream(StreamName=s)
        ...
        if shardCnt <= 1:
            evaluationList.append(build_evaluation(event['accountId'], "NON_COMPLIANT", event, annotation="Single Sharded Kinesis Data Stream"))
        else:
            evaluationList.append(
                build_evaluation(event['accountId'], "COMPLIANT", event, annotation="Multi-Sharded Kinesis Data Stream"))
...
def lambda_handler(event, context):
    ...
    evaluations = evaluate_compliance(event, configuration_item, valid_rule_parameters)
